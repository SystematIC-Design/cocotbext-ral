...
def gen_dv(obj, outdir, uvm_python=False):
    '''Generate DV files using a raw dict object parsed from hjson'''
    if uvm_python:
        gen_py(json_to_reg(obj), outdir)
    else:
...
def gen_py(block, outdir):
    '''Generate DV RAL model using uvm-python'''

    # Read template
    tpl_filename = resource_filename('reggen', 'uvm_reg.py.tpl')
    uvm_reg_tpl = Template(filename=tpl_filename)
    ...
    # Dump to output file
    dest_path = '{}/{}_ral.py'.format(outdir, block.name)
    with open(dest_path, 'w') as fout:
        fout.write(to_write)